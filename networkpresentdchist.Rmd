...
title("APS, AAAS, ANS", sub = "A bipartite layout of members and organizations",cex.main = 2,   font.main= 4, col.main= "blue", cex.sub = 0.75, font.sub = 3, col.sub = "red")
```
full_members
...
```{r}
plot(full_members$proj2, edge.width = E(full_members$proj2)$weight*0.2, layout = layout.fruchterman.reingold(full_members$proj2, weights = E(full_members$proj2)$weight),edge.label.cex=1,vertex.shape="circle",edge.color="red", vertex.size = 30, vertex.color="light blue", asp=0, margin=0,  vertex.label.dist = 0, vertex.label.cex = .75, vertex.label.color = "black")
title("National Learned Societies", sub = "Weighted by Shared Members",cex.main = 2,   font.main= 2, col.main= "black", cex.sub = 2, font.sub = 3, col.sub = "black")  
...
This is the good one to use in paper
```{r}
plot(members_col$proj2, edge.width = E(members_col$proj2)$weight*0.10, layout = layout.fruchterman.reingold(members_col$proj2, 
weights = E(members_col$proj2)$weight),edge.label.cex=1,vertex.shape="circle",edge.color="red", vertex.size = 25, vertex.color="lightblue", asp=0, margin=0,  vertex.label.dist = 0, vertex.label.cex = .5, vertex.label.color = "black")
title("Learned Societies", sub = "Weighted by Shared Members",cex.main = 1,   font.main= 2, col.main= "black", cex.sub = 1, font.sub = 3, col.sub = "black")  
...
```{r}
plot(members_dc$proj2, edge.width = E(members_dc$proj2)$weight*.3, layout = layout.fruchterman.reingold(members_dc$proj2, weights = E(members_dc$proj2)$weight),edge.label.cex=1,vertex.shape="circle",edge.color="red", vertex.size = 25, vertex.color="lightblue", asp=0, margin=0,  vertex.label.dist = 0, vertex.label.cex = .5, vertex.label.color = "black")
title("DC Learned Societies", sub = "Weighted by Shared Members",cex.main = 2,   font.main= 2, col.main= "black", cex.sub = 2, font.sub = 3, col.sub = "black")
```
...
#Load the data of the columbian institute and 1822 government and 1822 army This part is not working...

```{r}
dcgov = read.csv("~/Desktop/networkspresentation/data/1822_gov_col_army.csv")
```

Graph data frame of the "national" learned societies.
```{r}
dcgov_df <- graph.data.frame(dcgov, directed = FALSE)
plot (dcgov_df)
title("Network Graph of DC Gov", sub = "A network graph of members with name labels",cex.main = 2,   font.main= 4, col.main= "blue", cex.sub = 0.75, font.sub = 3, col.sub = "red")
```
...
```{r}
V(dcgov_df)
bipartite.mapping(dcgov_df)
V(dc_df)$type <- bipartite.mapping(dcgov_df)$type
is.bipartite(dcgov_df)
plot(dcgov_df, layout = layout.bipartite, vertex.label = NA)
title("DC Learned Societies", sub = "A bipartite layout of members and organizations",cex.main = 2,   font.main= 4, col.main= "black", cex.sub = 0.75, font.sub = 3, col.sub = "black")
```
full_members
...
Plot with DC Societies and Fed Govt 1822
```{r}
plot(members_dc$proj2, edge.width = E(members_dc$proj2), layout = layout.fruchterman.reingold(members_dc$proj2, weights = E(members_dc$proj2)$weight),edge.label.cex=1,vertex.shape="circle",edge.color="red", vertex.size = 25, vertex.color="lightblue", asp=0, margin=0,  vertex.label.dist = 0, vertex.label.cex = .5)
